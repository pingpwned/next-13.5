...
export const Plugin = ({
  ...
  const handleClick = (setting: any) => {
    setOpen({ name: setting, isOpen: !isOpen?.isOpen });
  };
  ...
  return (
    <>
      <PluginSettings
        ...
        setOpen={setOpen}
        settings={settings}
        pluginSettings={plugins.find((x: any) => x?.pluginId === settings.name)}
      />
      {settings.enabled && (
        <div className="mt-10 block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
          <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            {settings.name}
          </h5>
          <button
            className="w-full relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800"
            onClick={() => handleClick(settings.name)}
          >
