...
const PluginSettings = ({ isOpen, setting, setOpen }: any) => {
  ...
  const handleClick = () => {
    setModal(!modalOpen);
    setOpen({ name: setting.name, isOpen: !isOpen });
  };
  return (
    <div
      id="popup-modal"
...
export const Dashboard = ({ plugins }: { plugins: Plugin[] }) => {
  ...
  const handleClick = (setting: any) => {
    setOpen({ name: setting.name, isOpen: !isOpen?.isOpen });
  };
  return (
    <>
      {settings?.map((setting: any) => {
        return (
          <React.Fragment key={setting.name}>
            ...
            />
            {setting.enabled && (
              <div className="w-1/4 mt-10 block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                ...
                >
                  <span className="w-full flex justify-between relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                    Open settings
                    <svg
